...
import static com.mxw.crypto.Hash.hmacSha512;
import static com.mxw.crypto.Hash.sha256hash160;

/**
 * BIP-32 key pair.
 *
 * implementation and adapted from <a href="https://github.com/web3j/web3j/blob/master/crypto/src/main/java/org/web3j/crypto/Bip32ECKeyPair.java">Web3j</a>
 */
public class Bip32ECKeyPair extends ECKeyPair {
    ...
    private Bip32ECKeyPair deriveChildKey(int childNumber) {
        if (!hasPrivateKey()) {
            ...
            BigInteger ilInt = new BigInteger(1, il);
            Arrays.fill(il, (byte) 0);
            BigInteger privateKey = Numeric.toBigInt(getPrivateKey()).add(ilInt).mod(Sign.CURVE.getN());
    ...
    public ECPoint getPublicKeyPoint() {
        if (publicKeyPoint == null) {
            publicKeyPoint = Sign.publicPointFromPrivate(Numeric.toBigInt(getPrivateKey()));
        }
    ...
    public byte[] getPrivateKeyBytes33() {
        ...
        byte[] priv = bigIntegerToBytes32(Numeric.toBigInt(getPrivateKey()));
