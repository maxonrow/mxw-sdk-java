 ...
 *
 * implementation and adapted from <a href="https://github.com/web3j/web3j/blob/master/crypto/src/main/java/org/web3j/crypto/Sign.java">web3j</a>
 */
public class Sign {
    ...
    public static final ECNamedCurveParameterSpec CURVE_SPEC = ECNamedCurveTable.getParameterSpec("secp256k1");

    public static SignatureData signMessage(byte[] message, ECKeyPair keyPair, boolean needToHash) {
        BigInteger publicKey = Numeric.decodeQuantity(keyPair.getPublicKey());
        byte[] messageHash;
        if (needToHash) {
    ...
    public static BigInteger publicKeyFromPrivate(BigInteger privKey, boolean compressed) {
        ...
        return new BigInteger(Arrays.copyOfRange(encoded, 0, encoded.length)); // include prefix
    }
    ...
    public static BigInteger publicFromPoint(byte[] bits) {
        return new BigInteger(1, Arrays.copyOfRange(bits, 0, bits.length)); // include prefix
    }

    public static byte[] joinSignature(SignatureData signature) {
       // signature = splitSignature(signature);
        return Bytes.concat(signature.r,signature.s);
    }
